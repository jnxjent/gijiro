...
from kowake import transcribe_and_correct  # ✅ 追加：音声文字起こし用

load_dotenv()

DEPLOYMENT_ID = os.getenv("DEPLOYMENT_ID")

async def extract_meeting_info_and_speakers(audio_file_path: str, word_template_path: str = None) -> dict:
    """
    1. 音声ファイルの文字起こし（kowake使用）
    ...
    # -------------------------------------------
    try:
        print(f"[INFO] 音声ファイルの文字起こし開始: {audio_file_path}")
        full_transcription = await transcribe_and_correct(audio_file_path) or ""
        print("[OK] 全文文字起こしが完了")
    ...
    if table_labels:
        for label in table_labels:
            try:
                ...
                # 📌 **議題をリスト形式に修正**
                if label == "議題":
                    items = re.split(r"[、,]", extracted_value)  # カンマや読点で分割
